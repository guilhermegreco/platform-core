...
- **The spoke-ArgoCD equivalent turned out redundant in the PoC:** an `argocdAccessPolicy` resource *appeared* to work on rt-p5c only because the PoC aliases `argocdRoleARN == hubArgocdRoleARN` — so `hubAccessEntry` already grants that same principal cluster-admin (one access entry per cluster+principal). It was **removed** as redundant; `hubAccessEntry` covers the spoke's local deploys too. Restore a dedicated adopt-or-create entry when the spoke gets its own ArgoCD role (see the app-loop section below).
...
1. **Spoke ArgoCD deploy RBAC — covered by `hubAccessEntry` in the PoC (no separate resource).** The spoke's own ArgoCD needs cluster-admin to deploy the app loop locally. In the PoC `argocdRoleARN == hubArgocdRoleARN`, and an EKS access entry is unique per (cluster, principal), so `hubAccessEntry` (which grants that principal `AmazonEKSClusterAdminPolicy`) already covers it. A dedicated `argocdAccessPolicy` resource was therefore **removed as redundant** (it sat in `ACK.Terminal: already exists` on rt-p5c, managing the same entry). **Restore it** — an `adopt-or-create` `AccessEntry` on `argocdRoleARN`, exactly like `kroAccessPolicy` — the moment the spoke gets its **own distinct** ArgoCD IAM role (`argocdRoleARN != hubArgocdRoleARN`); then the two entries are genuinely separate and both are required. (kro's `kroAccessPolicy` is **not** redundant — it's the only grant for the kro principal.)
...
**Runtime RGD now GitOps-delivered (DONE — the hand-apply gap is closed).** The Runtime RGD (`team-eks`) had been hand-applied on every hub because `team-eks` had *never* published to ECR — its CI failed before publish. Getting a fully green, self-service pipeline surfaced **9 sequential CI/onboarding gaps** (the Runtime capability is the first whose CI must compile an Argo CD-referencing RGD, create a real EKS cluster, *and* auto-bump the umbrella via a GitHub App):
...
9. `bump-staging` had no GitHub App token — the eks repo was missing the `APP_ID`/`APP_PRIVATE_KEY` secrets that `create-github-app-token` needs (the other three capability repos got them on 2026-05-28; eks was created later). Fixed by setting both secrets on the eks repo from the existing `semver-bumper` App (App ID `3896472`; a freshly-generated private key — the App holds multiple keys, so this did not affect the other repos). NOTE: the App token targets **`platform-core`** (where the bump PR lands), so no App-installation change on the eks repo was needed — just the secrets.
With all 9 fixed the pipeline is **fully green end-to-end** — `lint → tier1 → real-infra-test → publish → bump-staging` — VERIFIED on run 28660384333: real spoke provisioned to ACTIVE + all 4 assertions + clean teardown, `team-eks` published to ECR, and the `semver-bumper` App auto-opened + auto-merged **PR #55 "chore(staging): bump team-eks to 0.0.2"** — the staging umbrella now pins `team-eks: 0.0.2`. `platform-capability-eks` is now **fully self-service through CI, identical to database/cache/pubsub**: a merge to its main tests a real spoke, publishes, and auto-bumps the umbrella; ArgoCD then delivers the Runtime RGD to each hub (tracking-id `argocd_platform-staging:...runtime.kro.run`, Active). A hub bootstrap is fully GitOps — no hand-applied `kubectl`.
- **Operational note:** the `semver-bumper` App now holds two private keys (2026-05-28 serves database/cache/pubsub; 2026-07-03 serves eks). Both must stay — deleting either breaks the repos using it.

**Still open for real fan-out (harden-later):** per-env AWS accounts + VPC quota; S3+locking remote state (local state + RETAIN orphans survivors); production umbrella `values.yaml` still references `plat-cp-*` subnet/cluster names (fine for a pure hub, harden before local data); **give each spoke its own distinct ArgoCD + kro/ACK IAM roles** (the PoC reuses the hub's `plat-cp-capability-*` roles — which is why `argocdRoleARN == hubArgocdRoleARN` and the dedicated spoke `argocdAccessPolicy` entry was dropped as redundant; restore it when the roles diverge); and swap the `AmazonEKSClusterAdminPolicy` grants for least-privilege access policies.

### Operating-model validation (multi-agent E2E run) — findings

A simulated run exercised the full spine: 3 capability "teams" (database/cache/pubsub) each shipped one small upgrade-safe feature through its own pipeline in parallel, a release captain promoted staging→production, a **live prod control-plane hub** was stood up in parallel, and the **staging hub launched a Runtime spoke** (two-layer model). What the spine proved, and the gaps it surfaced:

- **The seam holds.** Capability autonomy up to ECR + staging (cache 1.0.6, pubsub 1.0.7 shipped with zero platform involvement, auto-bumping `argocd/staging`), and the release captain gated on staging health then promoted via a reviewed `argocd/production` PR. Promotion landed **live** on `plat-cp-prod-cluster` — the promoted feature fields are present in the prod RGD schemas.
- **The pipeline is a real quality gate.** database's first feature (a 4th DBInstance **tag**) was rejected only at **tier2 by real ACK/RDS** — `tags: lengths of slices don't match` (an ACK-RDS tag-diff quirk: RDS auto-injects tags, so a desired-tag-slice-length mismatch throws; invisible to lint/tier1). STOP-on-failure fired; nothing bad published; the release captain **withheld** the whole promotion until staging was green (partial promotion = ship-what-passed). **Lesson: do NOT add extra tags to an ACK RDS `DBInstance`** — use an optional schema field wired to a real RDS field (e.g. `backupRetentionDays` → `backupRetentionPeriod`) instead, the same pattern cache/pubsub used.
- **FINDING — the runtimes umbrella is a SECOND, un-automated promotion surface.** `bump-staging` auto-bumps only `argocd/staging/Chart.yaml` (the hub). `argocd/runtimes/<env>/Chart.yaml` (what a Runtime *spoke* actually runs) is **never** auto-bumped — so a spoke receives the *old* capability versions until someone explicitly promotes them there. A staging runtime today runs 1.1.10/1.0.5/1.0.6 regardless of what staging-hub has. **Decision needed:** extend `bump-staging` (and the release-captain promotion) to also target `runtimes/<env>`, or document runtime promotion as a separate manual step.
- **FINDING — ArgoCD capability needs cluster-admin RBAC, and it was un-codified.** The managed ArgoCD capability's auto-created access entry (ArgoCD policies only) can't `list` arbitrary `*.services.k8s.aws` CRDs → cluster-cache fails → `platform-<env>` stuck SYNC=Unknown. Staging had `AmazonEKSClusterAdminPolicy` granted out-of-band; the from-code prod hub did not. **Fixed + codified** as `aws_eks_access_policy_association` in `platform-control-plane` (imported existing staging assoc; harden to least-privilege later). Note: a **spoke's** ArgoCD is the AWS-managed/IdC-backed one (control plane off-cluster), so it does **not** exhibit this cluster-scoped CRD-list gap — the gap is specific to the self-hosted hub ArgoCD.
...
| **kro (+ hub-ArgoCD) access-entry policy = `AmazonEKSClusterAdminPolicy`** via `adopt-or-create` (Runtime RGD `kroAccessPolicy`; `hubAccessEntry` covers the argocd principal in the PoC) | Proves the access-entry-native grant works end-to-end (rt-p5c/rt-p5d: Database→ACK SecurityGroup; app loop deployed locally) without in-cluster RBAC | Cluster-admin is far broader than needed. Replace with least-privilege cluster-access-policies (kro: `kro.run` + `*.services.k8s.aws`; argocd: read-cluster-wide + write-namespace). `kroAccessPolicy` **adopts** the EKS-auto-created entry and **adds** cluster-admin alongside the default `AmazonEKSKROPolicy` — document this co-ownership so it isn't a surprise on upgrade. When the spoke gets its own ArgoCD role, add a dedicated `argocdAccessPolicy` (dropped as redundant in the PoC — same principal as `hubAccessEntry`). |
